...
exports.getDashboardData = async (req, res) => {
  try {
    ...
    if (!pravasiBoys || pravasiBoys.length === 0) {
      return sendResponse(res, false, 'No users found in the system', {
        statistics: {
          totalPravasi: 0,
          verifiedPravasi: 0,
          unverifiedPravasi: 0
    ...
    const featuredUser = pravasiBoys.length > 0 ? {
      ...
      publicId: pravasiBoys[0].publicId,
      name: pravasiBoys[0].nameEn,
      bloodGroup: pravasiBoys[0].bloodGroupEn,
    ...
    return sendResponse(res, false, 'Internal server error occurred while fetching dashboard data. Please check your network connection and try again.', null, 500);
  }
};
...
exports.getPravasiBoysList = async (req, res) => {
  try {
    ...
    if (!users || users.length === 0) {
      return sendResponse(res, false, 'No pravasi boys found matching the criteria', {
        users: [],
        pagination: {
          currentPage: page,
          totalPages: 0,
    ...
    return sendResponse(res, false, 'Internal server error occurred while fetching pravasi boys list. Please check your network connection and try again.', null, 500);
  }
};