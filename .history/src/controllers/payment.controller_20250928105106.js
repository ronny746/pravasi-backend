...
exports.verifyPayment = async (req, res) => {
  try {
    ...
    // Find user by order ID or userId
    let user;
    if (userId) {
      user = await User.findById(userId);
    } else {
      user = await User.findOne({ "paymentInfo.orderId": razorpay_order_id });
    }
    ...
    try {
      ...
      user.paymentInfo.expiryAt = null; // lifetime access
      user.paymentInfo.paymentMethod = paymentDetails.method;
      ...
      if (paymentIndex !== -1) {
        ...
        user.paymentInfo.paymentHistory[paymentIndex].paymentMethod = paymentDetails.method;
        user.paymentInfo.paymentHistory[paymentIndex].signature = razorpay_signature;
      }
      ...
      // Still update the user if signature is valid
      user.paymentInfo.status = 'paid';
      user.paymentInfo.paymentId = razorpay_payment_id;
      user.paymentInfo.signature = razorpay_signature;
      user.isPaid = true;
...
exports.getPaymentHistory = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId).select("paymentInfo isPaid");
    if (!user) {
    ...
    return res.status(200).json({ 
      ...
      data: {
        ...
        currentPaymentStatus: user.paymentInfo?.status || 'not_paid',
        paymentHistory: paymentHistory.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
      }
    });
...
exports.checkPaymentStatus = async (req, res) => {
  try {
    ...
    const user = await User.findOne({ "paymentInfo.orderId": orderId }).select("paymentInfo");
    if (!user) {
      return res.status(404).json({ 
        success: false, 
        message: "Order not found" 
    ...
    const paymentRecord = user.paymentInfo.paymentHistory.find(p => p.orderId === orderId);
    ...
    return res.status(200).json({ 
      ...
      data: {
        orderId: orderId,
        status: user.paymentInfo.status,
        amount: user.paymentInfo.amount,
        paymentId: user.paymentInfo.paymentId || null,
...
exports.handlePaymentFailure = async (req, res) => {
  try {
    ...
    user.paymentInfo.status = 'failed';
    user.paymentInfo.errorCode = error_code;
    user.paymentInfo.errorDescription = error_description;

    // Update payment history
    const paymentIndex = user.paymentInfo.paymentHistory.findIndex(p => p.orderId === razorpay_order_id);
    if (paymentIndex !== -1) {
      user.paymentInfo.paymentHistory[paymentIndex].status = 'failed';
      user.paymentInfo.paymentHistory[paymentIndex].errorCode = error_code;
      user.paymentInfo.paymentHistory[paymentIndex].errorDescription = error_description;
      user.paymentInfo.paymentHistory[paymentIndex].updatedAt = new Date();
    }

    await user.save();

    console.log(`Payment failure recorded for user: ${user._id}`);
