...
exports.sendOtp = async (req, res) => {
  try {
    ...
    // Basic validation - only phone required
    if (!phone) {
...
//   }
// };
// UPDATE PROFILE - Enhanced with missing fields detection for first-time users
exports.updateProfile = async (req, res) => {
  try {
    const { userId } = req.params;
    if (!userId) {
    ...
    // 🔑 Merge DB user + updateFields to get current snapshot
    const snapshot = { ...user.toObject(), ...updateFields };
    ...
    const requiredFieldsMap = {
      ...
      email: "Email Address", 
      phone: "Phone Number",
      bloodGroupEn: "Blood Group",
      addressCurrentEn: "Current Address",
      addressPermanentEn: "Permanent Address",
      occupationTypeEn: "Occupation Type",
      occupationCompanyEn: "Company Name",
      occupationDesignationEn: "Designation",
    ...
    for (const [field, displayName] of Object.entries(requiredFieldsMap)) {
      ...
      if (!value || value.toString().trim() === "") {
        ...
        presentFields.push(displayName);
      }
    }

    // Set account completion status
    updateFields.isAccountCompleted = missingFields.length === 0;

    // Update user
    const updatedUser = await User.findByIdAndUpdate(userId, updateFields, { new: true, runValidators: true })
      .select("-otp -password");

    // Prepare response based on completion status
    let response = {
      success: true,
      statusCode: 200
    };

    if (updateFields.isAccountCompleted) {
      // Account is complete
      response.message = "Profile updated successfully! Your account is now complete.";
      response.data = {
        user: updatedUser,
        profileStatus: {
          isComplete: true,
          completionPercentage: 100,
          totalRequiredFields: Object.keys(requiredFieldsMap).length,
          completedFields: Object.keys(requiredFieldsMap).length
        }
      };
    } else {
      // Account is incomplete - show missing fields
      response.message = `Profile updated successfully! Please complete the remaining ${missingFields.length} field(s) to finish your profile.`;
      response.data = {
        user: updatedUser,
        profileStatus: {
          isComplete: false,
          completionPercentage: Math.round((presentFields.length / Object.keys(requiredFieldsMap).length) * 100),
          totalRequiredFields: Object.keys(requiredFieldsMap).length,
          completedFields: presentFields.length,
    ...
    // Add password update message if password was updated
    if (password) {
      response.message = response.message.replace("Profile updated", "Profile and password updated");
    }
