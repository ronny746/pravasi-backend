...
// UPDATE PROFILE - Enhanced to work with your model
exports.updateProfile = async (req, res) => {
  try {
    const { userId } = req.params;
    const {
    ...
    if (!user.phoneVerified) {
      return res.status(403).json({
        success: false,
        message: 'Please verify your phone number first before updating profile',
        statusCode: 403
      });
    }

    // Check if email/phone/username is being changed and already exists
    if (email && email !== user.email) {
      const existingEmailUser = await User.findOne({ email, _id: { $ne: userId } });
      if (existingEmailUser) {
        return res.status(409).json({
    ...
    if (username && username !== user.username) {
      ...
      if (existingUsernameUser) {
        return res.status(409).json({
          success: false,
          message: 'Username already exists with another account',
          statusCode: 409
        });
      }
    }
    ...
    // Set isVerified to true when profile is updated
