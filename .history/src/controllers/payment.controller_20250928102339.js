...
console.log('Cashfree Config:', {
  ...
  appId: CASHFREE_APP_ID ? `${CASHFREE_APP_ID.substring(0, 10)}...` : 'NOT_SET',
  secretKey: CASHFREE_SECRET_KEY ? `${CASHFREE_SECRET_KEY.substring(0, 10)}...` : 'NOT_SET',
...
const makeRequestWithRetry = async (url, data, maxRetries = 2) => {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.log('Request headers:', {
        'x-client-id': config.headers['x-client-id'] ? `${config.headers['x-client-id'].substring(0, 10)}...` : 'MISSING',
        'x-client-secret': config.headers['x-client-secret'] ? `${config.headers['x-client-secret'].substring(0, 10)}...` : 'MISSING',
