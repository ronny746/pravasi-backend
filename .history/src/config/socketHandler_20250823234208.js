...
class SocketHandler {
    ...
    io.on('connection', (socket) => {
      ...
      socket.on('updateStatus', async (data) => {
        try {
          ...
          console.log(`📊 ${socket.username} (${socket.userId}) status: ${status}`);
  ...
  // Get online users from database and connected users
  static async getOnlineUsers() {
    try {
      ...
      }).select('userId username photoUrl status lastSeen');
