const express = require('express');
const bodyParser = require('body-parser');
const googleTTS = require('google-tts-api');
const fs = require('fs');
const https = require('https');
const path = require('path');

const app = express();
app.use(bodyParser.json());

// Static folder to serve audio files
app.use('/audio', express.static(path.join(__dirname, 'audio')));

function splitText(text, maxLength = 200) {
  const chunks = [];
  let start = 0;
  while (start < text.length) {
    let end = start + maxLength;
    if (end < text.length) {
      while (text[end] !== ' ' && end > start) end--;
    }
    chunks.push(text.slice(start, end).trim());
    start = end + 1;
  }
  return chunks;
}

function downloadChunk(url, filePath) {
  return new Promise((resolve, reject) => {
    const file = fs.createWriteStream(filePath);
    https.get(url, (response) => {
      response.pipe(file);
      file.on('finish', () => file.close(resolve));
    }).on('error', (err) => {
      fs.unlinkSync(filePath);
      reject(err);
    });
  });
}

app.post('/tts-link', async (req, res) => {
  const { text, lang = 'en', speed = 1 } = req.body;
  if (!text) return res.status(400).json({ error: 'Text is required' });

  try {
    const chunks = splitText(text, 200);
    const chunkFiles = [];

    if (!fs.existsSync('audio')) fs.mkdirSync('audio');

    for (let i = 0; i < chunks.length; i++) {
      const url = googleTTS.getAudioUrl(chunks[i], { lang, slow: speed < 1 });
      const filePath = path.join('audio', `chunk_${Date.now()}_${i}.mp3`);
      await downloadChunk(url, filePath);
      chunkFiles.push(filePath);
    }

    // Merge audio using ffmpeg
    const listFile = path.join('audio', `files_${Date.now()}.txt`);
    fs.writeFileSync(listFile, chunkFiles.map(f => `file '${path.resolve(f)}'`).join('\n'));

    const outputFile = path.join('audio', `final_${Date.now()}.mp3`);
    const { exec } = require('child_process');

    exec(`ffmpeg -y -f concat -safe 0 -i ${listFile} -c copy ${outputFile}`, (err) => {
      // Cleanup chunk files and list file
      chunkFiles.forEach(f => fs.unlinkSync(f));
      fs.unlinkSync(listFile);

      if (err) return res.status(500).json({ error: 'Audio merge failed', details: err.message });

      // Return browser-friendly URL
      const audioUrl = `${req.protocol}://${req.get('host')}/audio/${path.basename(outputFile)}`;
      res.json({
        message: 'Audio generated successfully',
        audioUrl,
        downloadLink: audioUrl  // same URL works for download in browser
      });
    });

  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'TTS generation failed', details: err.message });
  }
});

app.listen(3000, () => console.log('Server running on http://localhost:3000'));
